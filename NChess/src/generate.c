...
generate_pieces_psudo_pawns(Board* board){
    uint64 op_map;
    int idx;
    ...
    if (Board_IS_WHITETURN(board)){
        ...
        LOOP_U64_T(Board_WHITE_PAWNS(board)){
            board->moves[idx] = (((NCH_GET_ROWIDX(idx) == 1) && NCH_CHKFLG(allowed_map, (0x010100ULL << idx))) ?
                                 (0x010100ULL << idx) : (NCH_NXTSQR_UP(NCH_SQR(idx)) & allowed_map))
        ...
        LOOP_U64_T(Board_BLACK_PAWNS(board)){
            board->moves[idx] = (((NCH_GET_ROWIDX(idx) == 6) && NCH_CHKFLG(allowed_map, 0x0080800000000000ULL >> (63 - idx))) ?
                                 (0x0080800000000000ULL >> (63 - idx)): NCH_NXTSQR_DOWN(NCH_SQR(idx)) & allowed_map)
...
NCH_STATIC_INLINE void
generate_pieces_psudo_others(Board* board){
...
generate_moves(Board* board){
    ...
    if (allowed_squares){
        generate_pieces_psudo_pawns(board);
        generate_pieces_psudo_others(board);
