...
_generate_pieces_psudo_pawns(Board* board, uint64 allowed_pieces){
    ...
    if (side == NCH_White){
        ...
        LOOP_U64_T(pawns_map){
            sqr = NCH_SQR(idx);
            board->moves[idx] = (NCH_CHKUNI(sqr, NCH_ROW2) && !NCH_CHKUNI((0x010100 << idx), allowed_map) ? 
                                 0x010100 << idx : NCH_NXTSQR_UP(sqr) & allowed_map)
        ...
        LOOP_U64_T(pawns_map){
            sqr = NCH_SQR(idx);
            board->moves[idx] = (NCH_CHKUNI(sqr, NCH_ROW7) && !NCH_CHKUNI((0x010100 << (idx - 16)), allowed_map) ? 
                                 0x010100 << (idx - 16): NCH_NXTSQR_DOWN(sqr) & allowed_map)
...
_generate_pieces_psudo_king(Board* board){
    ...
    LOOP_U64_T(moves){
        if (get_checkmap(board, side, king_idx, Board_ALL_OCC(board))){
...
generate_castle_moves(Board* board){
    ...
    if (Board_IS_WHITETURN(board)){
        if (Board_IS_CASTLE_WK(board) && !NCH_CHKUNI(Board_ALL_OCC(board), 0x6)
        ...
        if (Board_IS_CASTLE_WQ(board) && !NCH_CHKUNI(Board_ALL_OCC(board), 0x70)
...
int generate_moves(Board* board){
